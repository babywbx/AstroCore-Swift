...
enum DeltaT {
    ...
    static func deltaT(decimalYear y: Double) -> Double {
        ...
        if y < 1941 {
            // 1920–1941
            let t = y - 1920
            return horner(t, coeffs: [21.20, 0.84493, -0.076100, 0.0020936])
        }

        if y < 1961 {
            // 1941–1961
            let t = y - 1950
            return horner(t, coeffs: [29.07, 0.407, -1.0 / 233.0, 1.0 / 2547.0])
        }

        if y < 1986 {
            // 1961–1986
            let t = y - 1975
            return horner(t, coeffs: [45.45, 1.067, -1.0 / 260.0, -1.0 / 718.0])
        }
        ...
        if y < 2050 {
            // 2005–2050
            let t = y - 2000
            return horner(t, coeffs: [62.92, 0.32217, 0.005589])
        }
